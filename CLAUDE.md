...
- **Daily question selection is deterministic, not random.** `selectDailyQuestions` in `src/lib/claudle.ts` derives the day's questions from the date alone, so every visitor gets the same 5 questions on the same day. Don't swap this back to `Math.random()` — that would break the shared daily puzzle.
- **Selection is a rotating deck, not an independent draw per day.** The date becomes an epoch day index, which maps to a position in an endless stream of decks — each deck a full seeded shuffle (`mulberry32`) of the bank, walked 5 questions at a time. Every question is therefore drawn exactly once per deck before any repeats, giving a ~17.6-day cycle instead of the birthday-collision clustering an independent per-day shuffle produces (that version repeated a question within 3 days 16% of the time; this one never does). `BOUNDARY_GUARD` keeps the head of each deck clear of the previous deck's tail so repeats can't bunch up at a deck change; it scales down via `guardWidth` on small banks. Deck seeds come from `claudle-deck-<index>`, so **changing that string, `QUESTIONS_PER_SESSION`, or the bank size reshuffles every future day's questions.**
...
- **Devtools console commands.** `ClaudleGame` binds `window.claudle` while mounted: `claudle.reset()` wipes today's saved progress and restarts from question 1, `claudle.session()` dumps the stored session. Reset replays the *same* 5 questions — the day's set is fixed by the date — so it's for re-testing the flow, not for getting a fresh deal.
